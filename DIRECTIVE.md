...
JSDoc is parsed via the TypeScript Compiler API (`getJSDocParameterTags` / `getJSDocReturnTag`) — no separate `comment-parser` required for v1. Pin resolved versions in `package.json`.
